...
CREATE TABLE AuditLogs (
    ...
    AccountId SMALLINT NULL, -- Liên k?t ??n SystemAccount
    EntityName NVARCHAR(100) NOT NULL,
    Action NVARCHAR(50) NOT NULL,
    BeforeData NVARCHAR(MAX) NULL,
    AfterData NVARCHAR(MAX) NULL,
    Timestamp DATETIME DEFAULT GETDATE(),
    CONSTRAINT FK_AuditLogs_SystemAccount FOREIGN KEY (AccountId)
...
CREATE TABLE NewsView (
    ...
    NewsArticleId NVARCHAR(20) NOT NULL,
    ViewedById SMALLINT NULL, -- có th? null n?u ng??i xem là khách
    ViewedAt DATETIME NOT NULL DEFAULT GETDATE(),

    CONSTRAINT FK_NewsView_Article FOREIGN KEY (NewsArticleId) REFERENCES NewsArticle(NewsArticleId),
    CONSTRAINT FK_NewsView_User FOREIGN KEY (ViewedById) REFERENCES SystemAccount(AccountId)
);
